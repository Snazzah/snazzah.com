onresize = () => {
  document.querySelector('.landing').outerHTML = `<div class="landing" style="width: ${window.innerWidth}px; height: ${
    window.innerHeight
  }px">${document.querySelector('.landing').innerHTML}</div>`;
  if (document.querySelector('.nsfw'))
    document.querySelector('.nsfw').outerHTML = `<div class="nsfw" style="width: ${window.innerWidth}px; height: ${
      window.innerHeight
    }px">${document.querySelector('.nsfw').innerHTML}</div>`;
...
const TSL = {
  ...
  entryInnerHTML: (entry) =>
    `<img src="https://api.snaz.in/v1/telegram/${entry.name}/${entry.files[0]}"><div><span class="name">${entry.title}${
      entry.official ? `<span class="official-tag">Official</span>` : ''
    }</span><span class="name small">${entry.name}</span><span class="name small">${
      entry.files.length
    } Stickers</span></div>`
};
...
const DomManager = {
  findEntryDom: (e) => {
    ...
    for (const i in parents) {
      const p = parents[i];
      if (p instanceof HTMLDivElement && p.classList.contains('entry') && !p.classList.contains('in-entry-popout'))
        return p;
    }
    return null;
  },
...
onmousedown = (e) => {
  ...
  const entry = DomManager.findEntryDom(e.target);
  if (entry) {
    console.log(entry);
    ...
    const set = TSL.json.find((s) => s.name === entry.dataset.name);
    console.log(set);
    document.querySelector('.entry-popout-stickers').innerHTML = '';
    window.location.hash = set.name;
    set.files.forEach((f) => {
      document.querySelector(
        '.entry-popout-stickers'
      ).innerHTML += `<div class="sticker" style="background-image: url(https://api.snaz.in/v1/telegram/${set.name}/${f})">`;
    });
    document.querySelector(
      '.modal .entry img+div'
    ).innerHTML += `<a class="big" target="_blank" href="https://t.me/addstickers/${set.name}">Add Stickers</a>`;
  }
  ...
  if (
    e.target.parentNode &&
    e.target.parentNode.classList.contains('image-modal') &&
  ...
  if (e.target.classList.contains('sticker')) {
    ...
    window.location.hash = `${window.location.hash}/${
      e.target.style.backgroundImage.replace(/url\("(.+)"\)/, '$1').split('/')[6]
    }`;
  }

  const rel = DomManager.findReloadDom(e.target);
  if (rel) {
...
onload = (notforced) => {
  ...
  fetch('https://api.snaz.in/v1/telegram/')
    .then((res) => res.json())
    .then((json) => {
      console.log(json);
      TSL.json = json;
      ...
      if (!['_', ''].includes(window.location.hash.slice(1))) {
        ...
        const set = json.find((s) => s.name === hash[0]);
        if (!set) return;
        document.querySelector('body').classList.add('fade');
        document.querySelector('.modal').style.display = 'table';
        document.querySelector('.entry-popout').innerHTML = `<div class="entry in-entry-popout">${TSL.entryInnerHTML(
          set
        )}</div>`;
        document.querySelector('.entry-popout-stickers').innerHTML = '';
        set.files.forEach((f) => {
